...
# 중요) 이 함수를 호출하는 곳이 없다. args가 어떤 값인지 확인 불가능. json, num_classes, dir_pill_class_base, pill_dataset_class0/1 갖고 있음.
# 정정) 현재 파일 맨 아래에 주석처리 되어있었음
def make_pill_class_list(args):
    ...
    문제는 현재 갖고있는 pill_label_path_sharp_score은 테스트용 1000개에 해당하는 값밖에 없음 나머지 약에 대한 json 파일 생성할 수 있는지?
    ...
    args에서 default는 0.8, 0.1, 0.1 확인
...
rename_non_candidate_to_s_id는 디렉토리 이름을 S로 변경. 주석처리할지 고민 필요
'''
