...
class Dataset_Image(Dataset):
    def __init__(self, args, list_test_image, transform=None, target_transform=None, run_phase='train'):
        self.args = args
        self.transform = transform
        self.target_transform = target_transform
        # print('print Path :', Path(dir_dataset))
        self.list_images = list_test_image
        # print(self.list_images)
        # print([(png, png.name) for png in Path(dir_dataset).iterdir() if png.suffix == '.png'])
        self.run_phase = run_phase
...
def run_prediction_model_1(list_test_image):
    # print('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> run_search_model activate')
    job = 'resnet152'
    args = get_cli_args(job=job, run_phase='test', aug_level=0, dataclass='01')
    ...
    args.dataset_valid = Dataset_Image(args, list_test_image, transform=transform_normalize, run_phase='test' if args.run_phase == 'test' else 'valid')
    args.batch_size = len(args.dataset_valid)
    args.verbose = False
    print(f'valid dataset was loaded')
